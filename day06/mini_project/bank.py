...
class SavingsAccount(Account):
    def __init__(self, owner, number, balance=0, rate=None):
        ...
        # Use Singleton value if rate is not specified
...
class CurrentAccount(Account):
    def __init__(self, owner, number, balance=0, overdraft=None):
        super().__init__(owner, number, balance)
        # Use Singleton value if overdraft limit is not specified
    ...
    def withdraw(self, amount):
        ...
        # Name mangling to modify parent's private property
...
if __name__ == "__main__":
    os.chdir(os.path.dirname(__file__)) # Sets context for the audit log

    print("--- 1. Configuring Bank (Singleton) ---")
    config = BankConfig()
    config.savings_rate = 0.10 # Raising the default interest rate
    config.overdraft_limit = 2000 # Increasing default overdraft

    print("\n--- 2. Creating Accounts (Factory) ---")
    # Will use the new Singleton defaults since parameters are omitted
    almaz_acc = AccountFactory.create("savings", "Almaz", "1002", 1500) 
    dawit_acc = AccountFactory.create("current", "Dawit", "1003", 800)

    print("\n--- 3. Setting Up Observers ---")
